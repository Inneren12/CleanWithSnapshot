...
`c7d8e9f0a1b2_add_competitor_benchmarking.py` adds `competitors` and `competitor_metrics` for manual competitor benchmarks, and
...
| `alembic_rls_audit.ini` | CI audit config that applies the canonical RLS audit chain |
...
# If heads keep appearing, always merge ALL current heads into ONE merge revision before merging other PRs.
