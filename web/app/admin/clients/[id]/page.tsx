...
type ClientQualitySummaryResponse = {
  client_id: string;
...
export default function ClientDetailPage() {
  ...
  const [summary, setSummary] = useState<ClientQualitySummaryResponse | null>(null);
  ...
  const loadSummary = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`${API_BASE}/v1/admin/quality/clients/${clientId}/summary`, {
        headers: authHeaders,
      });
      if (!response.ok) {
        throw new Error(`Failed to load summary (${response.status})`);
      }
      const data = (await response.json()) as ClientQualitySummaryResponse;
      setSummary(data);
    } catch (err) {
      console.error("Failed to load client summary", err);
      setError("Unable to load client quality summary.");
    } finally {
      setLoading(false);
    }
  }, [authHeaders, clientId, password, username]);
  ...
  return (
    <div className="admin-page">
      <AdminNav links={navLinks} activeKey="clients" />
      <div className="card">
        <div className="card-header">
          <div>
            <h1>Client {clientId ? `#${clientId}` : ""}</h1>
            <p className="muted">Quality snapshot for the selected client.</p>
          </div>
          <Link className="btn btn-secondary" href="/admin/quality/reviews">
            View reviews
          </Link>
        </div>
            ...
            <div className="card admin-card">
              <div className="card-header">
                <div>
                  ...
                  <div className="muted">Recent reviews and complaints tied to this client.</div>
                </div>
                <Link className="btn btn-ghost" href={`/admin/quality/reviews?client_id=${clientId}`}>
                  Open quality
                </Link>
              </div>
              <div className="card-body">
                ...
                <div>
                  ...
                  {summary.last_review ? (
                    <div>
                      ...
                      <div className="muted">
                        {summary.last_review.worker_name ?? summary.last_review.worker_id ?? "Worker"}{" "}
