...
def test_default_team_dedupe_is_fk_safe(tmp_path):
    ...
    try:
        ...
        try:
            ...
            with engine.begin() as conn:
                ...
                conn.execute(
                    sa.text("INSERT INTO teams (team_id, name) VALUES (:id, :name)"),
                    {"id": 1, "name": "Default Team"},
                )
                conn.execute(
                    sa.text("INSERT INTO teams (team_id, name) VALUES (:id, :name)"),
                    {"id": 2, "name": "Default Team"},
                )
                conn.execute(
                    sa.text("INSERT INTO teams (team_id, name) VALUES (:id, :name)"),
                    {"id": 3, "name": "Crew A"},
    ...
    try:
        ...
        with engine.connect() as conn:
            teams = conn.execute(
                sa.text("SELECT name, COUNT(*) AS c FROM teams GROUP BY name")
            ).fetchall()
            team_map = {row.name: row.c for row in teams}
            assert team_map.get("Default Team") == 1
            assert team_map.get("Crew A") == 1

            booking_teams = conn.execute(
                sa.text("SELECT booking_id, team_id FROM bookings ORDER BY booking_id")
            ).fetchall()
