...
def test_e2e_proxy_allows_trusted_peer(client):
...
def test_e2e_proxy_rejects_untrusted_peer_with_reason(client_no_raise):
    _configure_proxy_settings()
...
def test_e2e_proxy_rejects_bad_signature_with_reason(client_no_raise):
    _configure_proxy_settings()
    settings.trusted_proxy_ips = ["testclient"]
    ...
    response = client_no_raise.get("/v1/admin/profile", headers=headers)
    assert response.status_code == 401
    assert response.headers["X-Admin-Auth-Fail-Reason"] == "bad_signature"
