...
async def _database_status(request: Request) -> dict[str, Any]:
    ...
    Returns a dict with keys expected by tests:
    - ok: bool
    - message: str
...
async def readyz(request: Request) -> JSONResponse:
    """Readiness endpoint for Kubernetes/Docker readiness probes.

    Checks:
    - Database connectivity (SELECT 1)
    - Alembic migration status (current vs expected heads)
    - Job heartbeat (if enabled via JOB_HEARTBEAT_REQUIRED)

    Returns HTTP 200 when ready, HTTP 503 otherwise.
    """
