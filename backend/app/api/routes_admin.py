...
def _resolve_admin_org(request: Request, identity: AdminIdentity) -> uuid.UUID:
    ...
    if requested_org_header:
        ...
        if saas_identity and requested_org != saas_identity.org_id:
            raise build_admin_forbidden_exception(reason="forbidden_org", detail="Forbidden")
...
def _parse_availability_scope(scope: str | None) -> tuple[str | None, int | None]:
    ...
    if ":" not in normalized:
        raise HTTPException(status_code=422, detail="Invalid scope format")
...
async def suggest_schedule(
    ...
    if resolved_end is None:
        if duration_min is None:
            raise HTTPException(status_code=422, detail="Missing ends_at or duration_min")
...
async def update_booking(
    ...
    if "ends_at" in fields_set:
        if payload.ends_at is None:
            raise HTTPException(status_code=422, detail="ends_at is required")
