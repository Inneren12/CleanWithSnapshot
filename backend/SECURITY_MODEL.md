...
### Admin MFA enforcement matrix

| Access path | Roles requiring MFA | MFA signal | Enforcement point | Notes |
| --- | --- | --- | --- | --- |
| `/v1/auth/login` + `/v1/auth/refresh` (SaaS) | Roles listed in `admin_mfa_required_roles` (defaults to `owner, admin`) when `admin_mfa_required=true` | `mfa_code` (TOTP) | `routes_auth.py` sets `mfa_verified` on session/JWT | MFA required only when the user has TOTP enabled; missing/invalid codes return 401 `type=mfa_required`. |
| `/v1/admin/*` + `/v1/iam/*` (SaaS JWT) | Roles listed in `admin_mfa_required_roles` when `admin_mfa_required=true` | `mfa_verified` claim on JWT | `AdminMfaMiddleware` (`app/api/mfa.py`) | Missing/false `mfa_verified` returns 401 `type=mfa_required`. |
| `/v1/admin/*` (legacy Basic Auth) | All legacy Basic Auth users | `X-Auth-MFA` header (`admin_proxy_auth_header_mfa`) | `AdminAccessMiddleware` (`app/api/admin_auth.py`) | Header must be `"true"` and request must be from a trusted proxy. Test-only bypass: header may be omitted when `settings.testing=true` or `APP_ENV` is `test/ci/e2e`. |
| `/v1/admin/*` (proxy auth) | All proxy-authenticated admins | `X-Auth-MFA` header (`admin_proxy_auth_header_mfa`) | `AdminAccessMiddleware` (`app/api/admin_auth.py`) | Header required for proxy auth in all environments; no test bypass. |
| `/v1/admin/break-glass/*` (SaaS) | `owner` or `security`; MFA required if role listed in `admin_mfa_required_roles` and `admin_mfa_required=true` | `mfa_verified` claim on JWT | `break_glass.py` + `AdminMfaMiddleware` | Break-glass start/revoke/review inherits `/v1/admin/*` checks; start adds explicit MFA gate for eligible SaaS roles. |

**Testing bypass**: The only MFA bypass is for legacy Basic Auth in test/CI/e2e modes (`_test_mfa_bypass_enabled` in `app/api/admin_auth.py`). Use this exclusively in automated tests; production/admin proxies must always send `X-Auth-MFA: true`.
...
- **Postgres row-level security**: RLS is enforced on **all org-scoped tables** (every table with an `org_id` column). The tenant key is always `org_id` (UUID), and policies use `org_id = NULLIF(current_setting('app.current_org_id', true), '')::uuid` so reads/writes are scoped to the request’s org. Each request sets `app.current_org_id` via `SET LOCAL` at transaction start for Postgres connections. Queries must still include explicit `org_id` filters for correctness and performance; RLS is a defense-in-depth safety net that blocks cross-org access if a filter is missed. SQLite runs without RLS for local testing.
- **Common RLS pitfalls**: forgetting to set `app.current_org_id` for background jobs/scripts (results in empty reads and blocked writes), adding a new org-scoped table without `org_id`/RLS, or using a superuser role that bypasses RLS. Use the application role (NOBYPASSRLS) and follow the RLS checklist in `docs/rls.md` for new tables.
