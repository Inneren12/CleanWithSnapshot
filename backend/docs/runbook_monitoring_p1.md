...
- `job_last_heartbeat_timestamp{job="jobs-runner"}` should be newer than 3 minutes (default `JOB_HEARTBEAT_TTL_SECONDS=180`); `job_last_success_timestamp` should advance at least every 15 minutes per job.
...
## What to do
- **Ready/health issues:** Hit `/readyz` and `/healthz` locally; check database connectivity and recent migrations. Verify `job_last_heartbeat_timestamp`, `job_heartbeat_age_seconds`, and `runner_id` in `/v1/admin/jobs/status`.
